<div class="flex-auto m-5 max-w-sm p-5 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
    ...
    <div class="flex flex-wrap p-0 ">
        ...
        <form class="flex flex-wrap p-0 w-full" action=" {{ route('motoristas.destroy', $attributes->get('motorista_id')) }}" method="POST">
            @csrf
            @method('DELETE')

            <button class="w-full focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" type="submit">Deletar</button>
        </form>
    </div>
</div>