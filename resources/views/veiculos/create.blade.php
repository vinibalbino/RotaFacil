<x-layout>
    <div class="p-6">
        <div class="bg-white shadow-lg shadow-gray-500/40 rounded-lg p-6">
            <h1 class="text-2xl font-bold mb-4">Adicionar Veículo</h1>

            <form action="{{ route('veiculos.store') }}" method="POST" class="flex-col space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="number" id="ano" name="ano" value="{{ old('ano' ) }}" required
                        class="border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2" />
                </div>

                <div>
                    <label for="data_aquisicao" class="block text-sm font-medium text-gray-700">Data de Aquisição:</label>
                    <input type="date" id="data_aquisicao" name="data_aquisicao" value="{{ old('data_aquisicao' ) }}" required
                        class="border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2" />
                </div>

                <div>
                    <label for="km_aquisicao" class="block text-sm font-medium text-gray-700">KM Rodados na Aquisição:</label>
                    <input type="number" id="km_aquisicao" name="km_aquisicao" value="{{ old('km_aquisicao' ) }}" required
                        class="border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2" />
                </div>
